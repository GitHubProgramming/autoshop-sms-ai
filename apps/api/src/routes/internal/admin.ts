...
export async function adminRoute(app: FastifyInstance) {
  ...
  app.get("/admin/overview", { preHandler: [adminGuard] }, async (_req, reply) => {
    const [
      ...
      query<{ billing_status: string; count: string }>(
        `SELECT billing_status, COUNT(*) as count FROM tenants GROUP BY billing_status`
      ),
      query<{ count: number }>(
        `SELECT COUNT(*)::int FROM tenants WHERE created_at > NOW() - INTERVAL '7 days'`
      ),
      query<{ status: string; count: number }>(
        `SELECT status, COUNT(*)::int as count FROM signup_attempts WHERE created_at > NOW() - INTERVAL '7 days' GROUP BY status`
      ),
      query<{ count: number }>(
        `SELECT COUNT(*)::int FROM conversations WHERE opened_at >= CURRENT_DATE`
      ),
      query<{ count: number }>(
        `SELECT COUNT(*)::int FROM appointments WHERE created_at >= CURRENT_DATE`
      ),
      query<{ count: number }>(
        `SELECT COUNT(*)::int FROM appointments WHERE calendar_synced = false AND created_at < NOW() - INTERVAL '1 hour'`
      ),
      query<{ count: number }>(
        `SELECT COUNT(*)::int FROM tenants t WHERE NOT EXISTS (SELECT 1 FROM tenant_phone_numbers tpn WHERE tpn.tenant_id = t.id AND tpn.status = 'active')`
      ),
      query<{ count: number }>(
        `SELECT COUNT(*)::int FROM tenants t WHERE NOT EXISTS (SELECT 1 FROM tenant_calendar_tokens tct WHERE tct.tenant_id = t.id)`
      ),
      query<{ count: number }>(
        `SELECT COUNT(*)::int FROM tenants WHERE billing_status = 'trial' AND trial_ends_at > NOW() AND trial_ends_at <= NOW() + INTERVAL '3 days'`
      ),
      query<{ count: number }>(
        `SELECT COUNT(*)::int FROM tenants WHERE conv_limit_this_cycle > 0 AND conv_used_this_cycle::float / conv_limit_this_cycle >= 0.8`
      ),
      query<{
        id: string; shop_name: string; owner_email: string;
        billing_status: string; plan_id: string | null; created_at: string;
      }>(
        `SELECT id, shop_name, owner_email, billing_status, plan_id, created_at FROM tenants ORDER BY created_at DESC LIMIT 10`
      ),
      query<{
        id: string; shop_name: string; owner_email: string;
        billing_status: string; trial_ends_at: string | null;
        conv_used_this_cycle: number; conv_limit_this_cycle: number;
        has_phone: boolean; has_calendar: boolean;
      }>(
        `SELECT t.id, t.shop_name, t.owner_email, t.billing_status, t.trial_ends_at,
      ...
      query<{
        id: string; tenant_id: string; shop_name: string | null;
        event_type: string; processed_at: string;
      }>(
        `SELECT be.id, be.tenant_id, t.shop_name, be.event_type, be.processed_at
         FROM billing_events be LEFT JOIN tenants t ON t.id = be.tenant_id
         ORDER BY be.processed_at DESC LIMIT 5`
      ),
      query<{
        id: string; tenant_id: string; shop_name: string;
        customer_phone: string; status: string; opened_at: string; turn_count: number;
      }>(
        `SELECT c.id, c.tenant_id, t.shop_name, c.customer_phone, c.status, c.opened_at, c.turn_count
    ...
    return reply.status(200).send({
      ...
      new_signups_7d:        newSignupsRows[0]?.count ?? 0,
      signup_attempts_7d:    signupAttemptsByStatusRows,
      conversations_today:   convsTodayRows[0]?.count ?? 0,
      bookings_today:        bookingsTodayRows[0]?.count ?? 0,
      failed_calendar_syncs: failedCalendarSyncsRows[0]?.count ?? 0,
      no_twilio:             noTwilioRows[0]?.count ?? 0,
      no_calendar:           noCalendarRows[0]?.count ?? 0,
      near_expiry:           nearExpiryRows[0]?.count ?? 0,
      high_usage:            highUsageRows[0]?.count ?? 0,
  ...
  app.get("/admin/tenants/:id", { preHandler: [adminGuard] }, async (request, reply) => {
    ...
    const [
      ...
      query(
        `SELECT id, email, auth_provider, google_sub, created_at FROM users WHERE tenant_id = $1 ORDER BY created_at DESC`,
        [id]
      ),
      query(`SELECT * FROM tenant_phone_numbers WHERE tenant_id = $1 ORDER BY provisioned_at DESC`, [id]),
      query(
        `SELECT id, calendar_id, connected_at, last_refreshed, token_expiry FROM tenant_calendar_tokens WHERE tenant_id = $1`,
        [id]
      ),
      query(
        `SELECT id, customer_phone, status, turn_count, opened_at, last_message_at, closed_at, close_reason
         FROM conversations WHERE tenant_id = $1 ORDER BY opened_at DESC LIMIT 20`,
        [id]
      ),
      query(
        `SELECT id, customer_phone, customer_name, service_type, scheduled_at, calendar_synced, google_event_id, created_at
         FROM appointments WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 10`,
        [id]
      ),
      query(
        `SELECT id, event_type, processed_at, payload FROM billing_events WHERE tenant_id = $1 ORDER BY processed_at DESC LIMIT 20`,
        [id]
      ),
      query(
        `SELECT id, event_type, actor, metadata, created_at FROM audit_log WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 20`,
        [id]
      ),
      query<{ conv_used_this_cycle: number; conv_limit_this_cycle: number; usage_pct: number }>(
        `SELECT conv_used_this_cycle, conv_limit_this_cycle,
    ...
    return reply.status(200).send({
      ...
      audit_log:      auditRows,
      usage_pct:      usageRows[0]?.usage_pct ?? 0,
    });
  });
  ...
  app.get("/admin/conversations", { preHandler: [adminGuard] }, async (request, reply) => {
    ...
    return reply.status(200).send({ count: conversations.length, conversations, page });
  });
  ...
  app.get("/admin/conversations/:id", { preHandler: [adminGuard] }, async (request, reply) => {
    ...
    const [convRows, messagesRows, appointmentRows] = await Promise.all([
      ...
      query(
        `SELECT id, direction, body, sent_at, tokens_used, model_version FROM messages WHERE conversation_id = $1 ORDER BY sent_at ASC`,
        [id]
      ),
  ...
  app.get("/admin/bookings", { preHandler: [adminGuard] }, async (request, reply) => {
    ...
    return reply.status(200).send({ count: bookings.length, bookings, page });
  });
  ...
  app.get("/admin/billing", { preHandler: [adminGuard] }, async (_req, reply) => {
    const billing = await query(
      `SELECT
         ...
         CASE WHEN t.conv_limit_this_cycle > 0
           ...
           ELSE 0
         END as usage_pct,
         t.current_period_start, t.cancel_at_period_end,
    ...
    return reply.status(200).send({ count: billing.length, billing });
  });
  ...
  app.get("/admin/integrations", { preHandler: [adminGuard] }, async (_req, reply) => {
    ...
    return reply.status(200).send({ count: integrations.length, integrations });
  });
  ...
  app.get("/admin/errors", { preHandler: [adminGuard] }, async (_req, reply) => {
    ...
    return reply.status(200).send({ count: errors.length, errors });
  });
  ...
  app.get("/admin/signup-attempts", { preHandler: [adminGuard] }, async (request, reply) => {
    ...
    return reply.status(200).send({ count: attempts.length, attempts });
  });
  ...
  app.get("/admin/audit", { preHandler: [adminGuard] }, async (request, reply) => {
    ...
    return reply.status(200).send({ count: events.length, events });
  });
}
