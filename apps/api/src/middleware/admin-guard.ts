 ...
 *
 * To grant admin access: add email to ADMIN_EMAILS and restart the API.
 * To revoke admin access: remove email from ADMIN_EMAILS and restart the API.
 *
 * Returns:
...
export async function adminGuard(
  ...
  // 1. Verify JWT — sets request.user on success
  try {
  ...
  // 2. Require ADMIN_EMAILS to be configured (read at call time so env changes
  //    take effect on restart without code changes)
  ...
  // 3. Check allowlist
  const { email } = request.user as { tenantId: string; email: string };
